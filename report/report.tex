...
We evaluate the trained model on the held-out test set (14,918 examples) using four complementary metrics that capture different aspects of summarization quality.
...
The test perplexity (31.74) is slightly better than the validation perplexity at epoch 10 (35.47), indicating that the model generalizes well to unseen data. The BLEU score of 15.85\% and ROUGE-1 of 48.31\% are reasonable for a model trained from scratch without any pretrained language model weights.
...
The model captures the general domain and intent of each function. For \texttt{print\_log}, the prediction ``Log a message to a logger'' is semantically close to the reference ``Print a log message to standard error,'' correctly identifying the logging purpose though missing the specific destination. For the download functions, the model recognizes the URL-related nature but confuses ``download'' with ``search.''

\section{Discussion and Limitations}

\subsection{Strengths}

The model demonstrates several positive characteristics. First, it converges steadily over 10 epochs without overfitting, as evidenced by the continuously decreasing validation loss. Second, the test perplexity improves over the validation perplexity, suggesting good generalization. Third, the ROUGE-1 F1 of 48.31\% indicates that nearly half of the unigrams in the reference are captured in the predictions, meaning the model learns the correct vocabulary for describing code.
...
\textbf{No pretrained weights.} Training from scratch with 77M parameters on 252K examples is data-limited compared to pretrained models like CodeT5 or CodeBERT, which leverage billions of tokens during pretraining. This accounts for much of the gap in specificity and fluency.

\textbf{Sensitivity to existing docstrings in the input.} When the \texttt{--file} mode in \texttt{summarize.py} is used on functions that already contain docstrings, the model produces noticeably better summaries. This is because the encoder receives the existing natural language description as part of its input and effectively paraphrases it. When the docstrings are removed, the model must rely entirely on the code structure and produces weaker, more generic outputs. This reveals that a portion of the model's apparent summarization quality on well-documented code comes from re-reading existing documentation rather than true code understanding — a limitation that is masked during standard evaluation since the CodeXGLUE test set does not include pre-existing docstrings in the \texttt{code} field.

\subsection{Possible Improvements}

Several directions could improve performance: (1) increasing the training data or augmenting it with additional Python repositories, (2) using beam search during evaluation, (3) experimenting with longer sequence lengths (256 or 512 tokens) at the cost of increased training time, and (4) applying label smoothing to the cross-entropy loss to improve generalization.
...
Bareerah Muslim & 7175312 & Data understanding, evaluation, layer \\
